		...
		<?php if ( ! empty( $terms ) ) : ?>
			<ul>
				<?php foreach ( $terms as $p_term ) :?>
					<?php $thumbnail_id = get_term_meta( $p_term->term_id, 'thumbnail_id', true ); ?>
					...
					<li data-heading="<?php echo isset( $p_term->heading ) ? esc_attr( $p_term->heading ) : ''; ?>">
						<a href="<?php echo esc_url( get_term_link( $p_term ) ); ?>">
							<?php echo '<span><img src="' . $image . '" alt="term-thumbnail" style="width:50px; display:inline-block"</span>';?>
