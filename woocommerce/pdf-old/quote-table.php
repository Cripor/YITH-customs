	...
	<table class="quote-table" cellspacing="0" cellpadding="6" style="width: 100%;" border="0">
		...
		if ( ! empty( $items ) ) :
			...
			foreach ( $items as $item ) :
				...
				$title = $_product->get_title() . '<br>' . preg_replace($regex, ' ', $category); //phpcs:ignore

				if ( $_product->get_sku() !== '' && ywraq_show_element_on_list( 'sku' ) ) {
					$sku_label = apply_filters( 'ywraq_sku_label', __( ' Prod. Code:', 'yith-woocommerce-request-a-quote' ) );
